...
module.exports.run = async (client, interaction, settings, queue, lang) => {
    ...
    const infoEmbed = new EmbedBuilder().setAuthor({ name: `${song.name}`, url: song.url, iconURL: elements.ICON_FLOPY }).setThumbnail(song.thumbnail || elements.BANNER_SECONDARY).addFields({ name: `**${lang.MESSAGE_SONG_AUTHOR}**`, value: `${song.uploader.name}`, inline: true }, { name: `**${lang.MESSAGE_SONG_VIEWS}**`, value: `${song.views.toString().replace(/(.)(?=(\d{3})+$)/g,"$1,")}`, inline: true }, { name: `**${lang.MESSAGE_SONG_LIKES}**`, value: `${song.likes.toString().replace(/(.)(?=(\d{3})+$)/g,"$1,")}`, inline: true }, { name: `**${lang.MESSAGE_SONG_DURATION}**`, value: `${durationBar}` }).setColor(elements.COLOR_FLOPY)
