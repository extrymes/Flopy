...
module.exports = {
    ...
    run: async (client, interaction, settings, queue, lang) => {
        ...
        if(!songs) return
        const items = songs.map((item, i) => { return { label: `${i + 1}. ${item.name.length > client.config.SONG_MAX_DISPLAY ? item.name.substr(0, client.config.SONG_MAX_DISPLAY).concat("...") : item.name}`, value: item.url } })
