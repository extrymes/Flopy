...
module.exports = {
	...
	run: async (client, interaction, guildData, queue, lang) => {
		...
		if (!client.handleCooldown("searchCommand", member.id, 4000))
	...
	subrun: async (client, interaction, subinteraction, results) => {
		...
		switch (subinteraction.customId) {
			...
			case "add":
				...
				if (library.find((item) => item.url === selectedResult.url))
					return client.sendErrorNotification(subinteraction, `${isPlaylist ? lang.ERROR_LIBRARY_PLAYLIST_ALREADY_ADDED : lang.ERROR_LIBRARY_SONG_ALREADY_ADDED}`);
				if (library.length >= config.LIBRARY_MAX_LENGTH)
					return client.sendErrorNotification(subinteraction, `${lang.ERROR_LIBRARY_LIMIT_REACHED}`);
				// Add item to library
		...
		const searchButtons = new ActionRowBuilder()
			.addComponents(
				...
				new ButtonBuilder()
					...
					.setLabel(`${lang.BUTTON_LIBRARY_ADD}`)
			);
