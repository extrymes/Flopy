...
module.exports = {
  ...
  run: async (client, interaction, guildData, queue, lang) => {
    ...
    if (!client.manageCooldown("join", member.id, 2000)) return client.sendErrorNotification(interaction, `${lang.ERROR_ACTION_NOT_POSSIBLE}`);
    try {
      await client.distube.voices.join(voiceChannel);
      client.sendNotification(interaction, `${lang.MESSAGE_VOICE_CHANNEL_JOINED}`);
    } catch (error) {
      const errorMessage = client.getErrorMessage(error.message, lang);
      client.sendErrorNotification(interaction, `${errorMessage}`);
    }
  }
}