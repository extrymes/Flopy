...
module.exports = {
  ...
  run: async (client, interaction, guildData, queue, lang) => {
    ...
    try {
      // Play or add item to the queue using member last query
      await client.distube.play(member.voice.channel, client.queries[member.id], { textChannel: channel, member: member, metadata: interaction });
    } catch (error) {
