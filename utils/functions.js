...
module.exports = (client) => {
  ...
  client.createDashboard = (guild, queue, lang) => {
    ...
    const formattedSongs = songs.slice(1, config.QUEUE_MAX_LENGTH_DISPLAY + 1).map((song, i) => { return `**${i + 1}.** ${song.name.length > config.SONG_NAME_MAX_LENGTH_DISPLAY ? song.name.substr(0, config.SONG_NAME_MAX_LENGTH_DISPLAY).concat("...") : song.name}` }).reverse().join("\n");
