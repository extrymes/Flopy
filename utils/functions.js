...
module.exports = client => {
    ...
    client.createDashboard = (guild, queue, lang) => {
        ...
        if(song) {
            const items = queue.songs.slice(1, client.config.QUEUE_MAX_DISPLAY + 1).map((item, i) => { return `${i + 1}. ${item.name.length > client.config.SONG_MAX_DISPLAY ? item.name.substr(0, client.config.SONG_MAX_DISPLAY).concat("...") : item.name}` }).reverse().join("\n")
            const dashboardContent = `**__${lang.DASHBOARD_QUEUE}__**\n${queue.songs.length - 1 > client.config.QUEUE_MAX_DISPLAY ? `**+${queue.songs.length - 1 - client.config.QUEUE_MAX_DISPLAY}**\n` : ""}${items || lang.DASHBOARD_QUEUE_NO_SONG}`
            const dashboardEmbed = new EmbedBuilder().setTitle(`[${song.formattedDuration}] ${song.name}`).setImage(song.thumbnail).setFooter({ text: `${lang.DASHBOARD_VOLUME} ${queue.volume}%${queue.repeatMode === 0 ? "" : queue.repeatMode === 1 ? ` | ${lang.DASHBOARD_REPEAT_SONG}` : ` | ${lang.DASHBOARD_REPEAT_QUEUE}`}${queue.autoplay ? ` | ${lang.DASHBOARD_AUTOPLAY_ON}` : ""}${queue.filters.size ? ` | ${lang.DASHBOARD_FILTERS} ${queue.filters.size}` : ""}` }).setColor(guild.members.me.displayHexColor.replace("#000000", elements.COLOR_WHITE))
            ...
            const dashboardEmbed = new EmbedBuilder().setTitle(`${lang.DASHBOARD_SONG_NO_PLAYING}`).setDescription(`[Flopy](${elements.INVITE_FLOPY}) | [Flopy 2](${elements.INVITE_FLOPY2}) | [Flopy 3](${elements.INVITE_FLOPY3}) | [Support](${elements.INVITE_SUPPORT})`).setImage(elements.BANNER_DASHBOARD).setColor(elements.COLOR_FLOPY)
