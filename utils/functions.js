...
module.exports = (client) => {
  ...
  client.createDashboard = (guild, queue, lang) => {
    ...
    const formattedSongs = queue?.songs?.slice(1, config.QUEUE_MAX_LENGTH_DISPLAY + 1).map((song, i) => { return `**${i + 1}.** ${song.name.length > config.SONG_NAME_MAX_LENGTH_DISPLAY ? song.name.substr(0, config.SONG_NAME_MAX_LENGTH_DISPLAY).concat("...") : song.name}` }).reverse().join("\n");
    const dashboardContent = `**__${lang.DASHBOARD_QUEUE}__**\n${currentSong && queue.songs.length - 1 > config.QUEUE_MAX_LENGTH_DISPLAY ? `**+${queue.songs.length - 1 - config.QUEUE_MAX_LENGTH_DISPLAY}**\n` : ""}${currentSong ? (formattedSongs || lang.DASHBOARD_QUEUE_NO_SONG) : lang.DASHBOARD_QUEUE_NONE}`;
