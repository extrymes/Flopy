...
module.exports = client => {
    ...
    client.createDashboard = (guild, queue, lang) => {
        ...
        if(song) {
            const songs = queue.songs.slice(1, client.config.QUEUE_MAX_DISPLAY + 1).map((item, i) => { return `${i + 1}. ${item.name.length > client.config.SONG_MAX_DISPLAY ? item.name.substr(0, client.config.SONG_MAX_DISPLAY).concat("...") : item.name}` }).reverse().join("\n")
