...
module.exports = (client) => {
	...
	client.createGuildData = async (guild) => {
		...
		if (!guildData)
			throw new Error("Error when creating guild data!");
	...
	client.getGuildData = async (guild) => {
		...
		if (!guildData)
			return client.createGuildData(guild);
		return guildData.flopy1;
	}
	...
	client.updateGuildData = async (guild, data) => {
		...
		if (result.modifiedCount === 0)
			throw new Error("Error when updating guild data!");
		return true;
	}
	...
	client.createUserData = async (user) => {
		...
		if (!userData)
			throw new Error("Error when creating user data!");
	...
	client.getUserData = async (user) => {
		...
		if (!userData)
			return client.createUserData(user);
		return userData;
	}
	...
	client.checkMessageIsSendable = (guild, channel) => {
		...
		if (channel.viewable && channel.permissionsFor(member).has("SendMessages") && channel.permissionsFor(member).has("EmbedLinks") && Date.now() > member.communicationDisabledUntil)
			return true;
		return false;
	}
	...
	client.sendNewGuildMessage = (guild) => {
		...
		const firstEmbed = new EmbedBuilder()
			.setTitle("Get ready to listen to music easily!")
			.setDescription(`To get started, use \`/setup\` command in a channel.\nIf you need help, here is the [support server](${elements.INVITE_SUPPORT}).`)
			.setImage(elements.BANNER_FLOPY)
	...
	client.sendHelpMessage = (guild, channel, lang) => {
		const helpEmbed = new EmbedBuilder()
			.setAuthor({ name: `${lang.HELP_MESSAGE}`, iconURL: elements.ICON_FLOPY })
			.setDescription(`${client.dashboards[guild.id] ? lang.HELP_PLAY_SONG.replace("$channel", `${client.dashboards[guild.id].channel}`) : lang.HELP_SETUP_DASHBOARD.replace("$command", `\`/setup\``)}`)
			.setColor(elements.COLOR_FLOPY);
	...
	client.sendNotification = (destination, title, options) => {
		...
		if (!destination.token)
			return destination.send({ embeds: [notificationEmbed] }).then((message) => setTimeout(() => message?.delete().catch((error) => { }), 4000)).catch((error) => { });
		if (options?.editReply)
			destination.editReply({ embeds: [notificationEmbed], ephemeral: options?.ephemeral }).catch((error) => { });
		else
			destination.reply({ embeds: [notificationEmbed], ephemeral: options?.ephemeral }).catch((error) => { });
	...
	client.sendErrorNotification = (destination, title, options) => {
		const notificationEmbed = new EmbedBuilder()
			.setTitle(`${title}`)
			.setColor(elements.COLOR_GREY);
		if (!destination.token)
			return destination.send({ embeds: [notificationEmbed] }).then((message) => setTimeout(() => message?.delete().catch((error) => { }), 4000)).catch((error) => { });
		if (options?.editReply)
			destination.editReply({ embeds: [notificationEmbed], ephemeral: true }).catch((error) => { });
		else
			destination.reply({ embeds: [notificationEmbed], ephemeral: true }).catch((error) => { });
		setTimeout(() => destination.deleteReply().catch((error) => { }), 4000);
	}
	...
	client.createDashboard = (guild, queue, lang) => {
		...
		const dashboardEmbed = new EmbedBuilder()
			.setTitle(currentSong ? `[${currentSong.formattedDuration}] ${currentSong.name}` : `${lang.DASHBOARD_SONG_NO_PLAYING}`)
			.setDescription(!currentSong ? `[Flopy](${elements.INVITE_FLOPY}) | [Flopy 2](${elements.INVITE_FLOPY2}) | [Flopy 3](${elements.INVITE_FLOPY3}) | [Support](${elements.INVITE_SUPPORT})` : null)
			.setImage(currentSong ? currentSong.thumbnail : elements.BANNER_DASHBOARD)
			.setFooter(currentSong ? { text: `${lang.DASHBOARD_VOLUME} ${queue.volume}%${queue.repeatMode === 1 ? ` | ${lang.DASHBOARD_REPEAT_SONG}` : queue.repeatMode === 2 ? ` | ${lang.DASHBOARD_REPEAT_QUEUE}` : ""}${queue.autoplay ? ` | ${lang.DASHBOARD_AUTOPLAY_ON}` : ""}${queue.filters.size > 0 ? ` | ${lang.DASHBOARD_FILTERS} ${queue.filters.size}` : ""}` } : null)
			.setColor(currentSong ? guild.members.me.displayHexColor.replace("#000000", elements.COLOR_WHITE) : elements.COLOR_FLOPY);
		const dashboardButtons = new ActionRowBuilder()
			.addComponents(
				queue?.paused ? new ButtonBuilder()
					.setCustomId("resume")
					.setStyle(ButtonStyle.Primary)
					.setEmoji(elements.EMOJI_PLAY)
				: new ButtonBuilder()
					.setCustomId("pause").setStyle(ButtonStyle.Secondary)
					.setEmoji(elements.EMOJI_PAUSE)
					.setDisabled(!currentSong),
				new ButtonBuilder()
					.setCustomId("stop")
					.setStyle(ButtonStyle.Secondary)
					.setEmoji(elements.EMOJI_STOP)
					.setDisabled(!currentSong),
				new ButtonBuilder()
					.setCustomId("skip")
					.setStyle(ButtonStyle.Secondary)
					.setEmoji(elements.EMOJI_SKIP)
					.setDisabled(!currentSong),
				new ButtonBuilder()
					.setCustomId("repeat")
					.setStyle(queue?.repeatMode > 0 ? ButtonStyle.Primary : ButtonStyle.Secondary)
					.setEmoji(elements.EMOJI_REPEAT).setDisabled(!currentSong),
				new ButtonBuilder()
					.setCustomId("volume")
					.setStyle(ButtonStyle.Secondary)
					.setEmoji(elements.EMOJI_VOLUME)
					.setDisabled(!currentSong)
			);
	...
	client.handleCooldown = (name, target, time) => {
		...
		if (client.cooldowns[cooldownId])
			return false;
	...
	client.getErrorMessage = (error, lang) => {
		for (const { keywords, message } of config.DISTUBE_ERROR_MAPPING)
		{
			if (keywords.some((keyword) => error.includes(keyword)))
				return `${lang[message]}`;
		}
		console.error(error);
